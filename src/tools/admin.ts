...
export function registerAdminTool(server: McpServer, config: Config): void {
  server.tool(
    ...
    {
      action: z.enum(['status', 'restart', 'reset', 'channels', 'channel_allowlist_add', 'channel_allowlist_remove', 'users', 'allowlist_add', 'allowlist_remove', 'set_presence', 'kick', 'timeout', 'remove_timeout']).describe('The administrative action to perform.'),
      query: z.string().optional().describe('Optional channel/user name, mention, ID, or partial string to filter discovery actions.'),
      channel_id: z.string().optional().describe('Explicit Discord channel ID for reset or channel allowlist actions.'),
