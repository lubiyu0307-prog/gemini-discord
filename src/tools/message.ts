...
export function registerMessageTool(server: McpServer, config: Config): void {
  server.tool(
    ...
    ].join('\n'),
    {
      ...
      content: z.string().optional().describe('Message text. For "send"/"reply": the text to deliver to the OTHER channel (never use this for your normal response — that streams automatically to the origin channel). For "edit": the new message content. For "thread": the thread name.'),
