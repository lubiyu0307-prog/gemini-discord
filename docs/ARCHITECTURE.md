...
- **Density Selection**: Started/progress-only tool rows are suppressed while still updating the live header. Simple reads, searches, globs, and directory listings render as one-line rows with canonical labels. Shell output, logs, errors, MCP calls, edits, writes, web fetches, and meaningful results render as plain transcript messages with fenced blocks where useful. Successful shell calls suppress internal working-directory/execution-note metadata and show actual stdout/stderr. File writes and edits extract code previews from diffs instead of pasting diff metadata inline. Long sanitized output is attached as a text file instead of pasted inline.
