...
- **Console Layout**: Trace output uses a Gemini CLI-style rhythm inside Discord: prompt echo, phase line, one editable run header, compact completed tool rows, fenced output blocks, and a separate final assistant answer.
- **Density Selection**: Started/progress-only tool rows are suppressed while still updating the live header. Simple reads, searches, globs, and directory listings render as one-line rows with canonical labels. Shell output, logs, errors, MCP calls, edits, writes, web fetches, and meaningful results render as plain transcript messages with fenced blocks where useful. Successful shell calls suppress internal working-directory/execution-note metadata, show actual stdout/stderr, and keep compound commands to one visible shell row. File writes extract small code previews, while edits prefer compact diff hunks instead of full-file dumps. Long sanitized output is attached as a text file instead of pasted inline.
