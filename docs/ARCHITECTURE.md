...
- **Console Layout**: Trace output uses a Gemini CLI-style rhythm inside Discord: prompt echo, phase line, one editable run header, compact completed tool rows, fenced output blocks, and a quoted final assistant answer.
- **Density Selection**: Started/progress-only tool rows are suppressed while still updating the live header. Simple reads, searches, globs, and directory listings render as one-line rows with canonical labels. Title-only web fetches and skill activations render as compact header/result cards. Shell output, logs, errors, MCP calls, edits, web fetches, and meaningful results render as plain transcript messages with fenced blocks where useful. Successful shell calls suppress internal working-directory/execution-note metadata, show actual stdout/stderr, and keep compound commands to one visible shell row. File writes stay as compact Accepted/Created/Updated rows, while edits prefer compact diff hunks instead of full-file dumps. Long sanitized output is attached as a text file instead of pasted inline.
- **Discord Message Edits**: To stay within Discord rate limits, the `TraceDispatcher` keeps track of the run header and active tool messages and updates them in place as the workflow transitions from queued to running to complete or failed. Workflow final responses are sent through the same mention-safe, chunked dispatcher path. Tool updates correlate on both top-level `toolCallId` and older nested `toolCall.id`, so started, progress, and completion events edit the same trace message.
